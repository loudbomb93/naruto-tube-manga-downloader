...
(function () {
  ...
  window.checkFinal = function (final) {
    if (
      aIMGUrl.length === Object.keys(final).length &&
      !inIframe() &&
      aIMGUrl.length > 0
    ) {
      console.log(
        "Geladen: " + Object.keys(final).length + " von " + aIMGUrl.length
      );
      createpdf();
    } else if (!elem.is("body") && !inIframe()) {
      //elem.text("Lade: " + Object.keys(final).length + " von "+ aIMGUrl.length);
      console.log(
        "Lade: " + Object.keys(final).length + " von " + aIMGUrl.length
      );
    }
  };
  /**----------------------------------------------------------
  ...
  if (inIframe()) {
    ...
    if (sTitle === "500 Internal Server Error" || sTitle === "404 Not Found") {
      ...
      parent.window.postMessage(
        [sPage, "found", imgData, bwidth, bheight],
        "*"
      );
    }
  }
  ...
  if (
    document.location.hostname === sParenthostname &&
    $(".sagatable .list tbody").length !== 0
  ) {
    if (sParenthostname === "onepiece-tube.com") {
      $(".sagatable .list tbody")
        .find("[onclick]")
        .parent("tr")
        .each(function () {
          ...
          $(this)
            .find("td:eq(2)")
            .after(
              '<td> <button class="btn success LoudBomb-btn" >Download</button> </td>'
            );
        });
    } else {
      $(".sagatable .list tbody")
  ...
  $(".LoudBomb-btn").click(function () {
    ...
    if (typeof oInfo !== "undefined" && Object.keys(oInfo).length !== 0) {
      ...
      sKapitel = /^\d{1,4}$/g.test(oInfo[0].innerText)
        ? oInfo[0].innerText
        : false;
      ...
      for (var keys = 1; keys <= iSeiten; keys++) {
        var sSeite =
          keys.toString().length === 2 ? keys.toString() : "0" + keys;
