...
(function () {
  ...
  function displayMessage(evt) {
    if (
      ...
      if (
        typeof pagedata.number_of_pages !== "undefined" &&
        window.number_of_pages === 0
      ) {
      ...
      console.log(
        "Converting: " + pagedata.current_page + "/" + window.number_of_pages
      );
      if (pagedata.current_page <= pagedata.number_of_pages) {
        var curr_src = $("#lb_info_iframe")[0].src;
        $("#lb_info_iframe")[0].src = curr_src.replace(
          /\/[^\/]*$/,
  ...
  if (inIframe()) {
    ...
    for (var i = 0; i < headingnames.length; i++) {
      ...
      headingnames[i].parentNode.insertBefore(
        dwheading,
        headingnames[i].nextSibling
      );
    }
